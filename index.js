...
$.ajax({
    ...
    success: function(response){
        	$('.info').append('<div class="photo_user"><img class="img"  src="'+ response.data.profile_picture +'"></div><div class="name" >'+response.data.username+'</div><div class="count"><small>Kоличество публикаций</small>' + response.data.counts.media + '</div>');
    },
...
function load_inst_photo(){
	...
	var count = 0;     			                     // счетчик, что бы подружать 5 фотографий
	$.ajax({
	    ...
	    success: function(data){
	        for( x in data.data ){
	        	...
	        	// чтобы знать сколько фотографий уже подгружена и от этого индекса x начинать вставлять фотографии в документ 
					count++;
		        	var load_photo = '<div class="photo_inst"><img class="img_inst" id="'+ x +' "src="'+data.data[x].images.standard_resolution.url+'"></div>';
		        	//загружаются описания под фото
		        	if(data.data[x].caption){
		        		var load_text = '<div class="photo_desc"><p>' + data.data[x].caption.text + '</p></div>';
		        	} else{
		        		var load_text = '<div class="photo_desc" style="height:15px"></div>';  //если описания фото нет, то остается пустая строка
		        	}

		        	//Загружаются теги фотографии 
		        	var tag = data.data[x].tags;
		        	...
		        	var load_tags = '<div class="tag"><p>' + tag.join(" ") + '</p></div>';
		            //Количество комментариев 
		            var comments = '<i class="fa fa-comment"></i><div class="count_comment">'+ data.data[x].comments.count+'</div>';
		            console.log(comments);
		        	//Загружаются лайки фото 
		        	if(data.data[x].likes.count){
		        		var load_likeCount = '<div class="like"><i class="fa fa-heart"></i><div class="count_like">' + data.data[x].likes.count + '</div>'+comments+'</div>';
		        	}else{ 
		        		var load_likeCount = '<div class="like"><i class="fa fa-heart"></i><div class="count_like">' +  "0" + '</div></div>';  
		        	}
		            ...
		            var time = '<div class="date">' + date +'.'+ month +'.'+year + '</div>';


                    // Вывод всего html и подруженным данных 
		            $('.posts').append('<div class="photo">' + load_photo +'<div class="description" style="font-size:14px">'+ load_likeCount  + load_text + load_tags + time +'</div></div>');
		        }
		    }
	    },
	    error: function(data){
